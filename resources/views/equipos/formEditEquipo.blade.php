        ...
        <div class="wrapper">
            <div class="inner">
				...
				<form action="/empleado/equipos" method="POST" enctype="multipart/form-data">
                    @csrf
					<h3>Editar equipo</h3>
                    <p>ID del del equipo: {{$id}}</p>
                    <br>
                    <input type="text" name="nombre" class="form-control" placeholder="Nombre" value = "{{$equipo->nombre}}">
                    @error('nombre')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <br>
                    <label for="costo">$</label>
                    <input type="text" name="costo" class="form-control" placeholder="Costo x dia" value = "{{$equipo->costo_x_dia}}" style="display: inline">
                    @error('costo')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <br><br>
					<textarea name="descripcion" placeholder="Descripcion" class="form-control" style="height: 130px;">{{old($equipo->descripcion)}}</textarea>
                    @error('descripcion')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <div class="form-row">
